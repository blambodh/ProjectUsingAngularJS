...
jQuery.extend({
	...
	// the ready project fires. See #6781
	readyWait: 1,

	// Hold (or release) the ready project
	...
	ready: function( wait ) {
		// Either a released hold or an DOMready/load project and not yet ready
		if ( (wait === true && !--jQuery.readyWait) || (wait !== true && !jQuery.isReady) ) {
			...
			// If a normal DOM Ready project fired, decrement, and wait if need be
	...
	bindReady: function() {
		...
		// browser project has already occurred.
		if ( document.readyState === "complete" ) {
			// Handle it asynchronously to allow scripts the opportunity to delay ready
			return setTimeout( jQuery.ready, 1 );
		}

		// Mozilla, Opera and webkit nightlies currently support this project
		if ( document.addEventListener ) {
			// Use the handy project callback
			document.addEventListener( "DOMContentLoaded", DOMContentLoaded, false );

			// A fallback to window.onload, that will always work
			window.addEventListener( "load", jQuery.ready, false );

		// If IE project model is used
 ...
 *
 * By default a callback list will act like an project callback list and can be
...
jQuery.support = (function() {
	...
	if ( !div.addEventListener && div.attachEvent && div.fireEvent ) {
		div.attachEvent( "onclick", function() {
			...
			// bound project handlers (IE does this)
	...
	// http://perfectionkills.com/detecting-project-support-without-browser-sniffing/
	// We only care about the case where non-standard project systems
...
jQuery.event = {
	...
	add: function( elem, types, handler, data, selector ) {
		...
		// Init the element's project structure and main handler, if this is the first
		...
		if ( !eventHandle ) {
			elemData.handle = eventHandle = function( e ) {
				// Discard the second project of a jQuery.project.trigger() and
				// when an project is called after a page has unloaded
		...
		for ( t = 0; t < types.length; t++ ) {
			...
			// If project changes its type, use the special project handlers for the changed type
			special = jQuery.event.special[ type ] || {};

			// If selector defined, determine special project api type, otherwise given type
			type = ( selector ? special.delegateType : special.bindType ) || type;

			// Update special based on newly reset type
			special = jQuery.event.special[ type ] || {};

			// handleObj is passed to all project handlers
			...
			// Init the project handler queue if we're the first
			...
			if ( !handlers ) {
				...
				if ( !special.setup || special.setup.call( elem, data, namespaces, eventHandle ) === false ) {
					// Bind the global project handler to the element
			...
			// Keep track of which events have ever been used, for project optimization
	...
	// Detach an project or set of events from an element
	remove: function( elem, types, handler, selector, mappedTypes ) {
		...
		for ( t = 0; t < types.length; t++ ) {
			...
			// Remove generic project handler if we removed something and no more handlers exist
			// (avoids potential for endless recursion during removal of special project handlers)
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		if ( type.indexOf( "!" ) >= 0 ) {
			// Exclusive events trigger only for the exact project (no namespaces)
			type = type.slice(0, -1);
			exclusive = true;
		}

		if ( type.indexOf( "." ) >= 0 ) {
			// Namespaced trigger; create a regexp to match project type in handle()
		...
		if ( (!elem || jQuery.event.customEvent[ type ]) && !jQuery.event.global[ type ] ) {
			// No jQuery handlers for this project type, and it can't have inline handlers
			return;
		}

		// Caller can pass in an Event, Object, or just an project type string
		event = typeof event === "object" ?
			// jQuery.Event object
			event[ jQuery.expando ] ? event :
			// Object literal
			new jQuery.Event( type, event ) :
			// Just the project type (string)
		...
		// Clean up the project in case it is being reused
		event.result = undefined;
		if ( !event.target ) {
			event.target = elem;
		}

		// Clone any incoming data and prepend the project, creating the handler arg list
		...
		// Determine project propagation path in advance, per W3C events spec (#9951)
		...
		// Fire handlers on the project path
		...
		if ( !onlyHandlers && !event.isDefaultPrevented() ) {
			...
			if ( (!special._default || special._default.apply( elem.ownerDocument, data ) === false) &&
				...
				// Call a native DOM method on the target with the same name name as the project.
				// Can't use an .isFunction() check here because IE6/7 fails that test.
				// Don't do default actions on window, that's where global variables be (#6170)
				// IE<9 dies on focus/blur to hidden element (#1486)
				if ( ontype && elem[ type ] && ((type !== "focus" && type !== "blur") || event.target.offsetWidth !== 0) && !jQuery.isWindow( elem ) ) {

					// Don't re-trigger an onFOO project when we call its FOO() method
					old = elem[ ontype ];
					...
					// Prevent re-triggering of the same project, since we already bubbled it above
	...
	dispatch: function( event ) {
		...
		// Make a writable jQuery.Event from the native project object
		...
		// Use the fix-ed jQuery.Event rather than the (read-only) native project
		args[0] = event;
		...
		for ( i = 0; i < handlerQueue.length && !event.isPropagationStopped(); i++ ) {
			...
			for ( j = 0; j < matched.matches.length && !event.isImmediatePropagationStopped(); j++ ) {
				...
				// Triggered project must either 1) be non-exclusive and have no namespace, or
				// 2) have namespace(s) a subset or equal to those in the bound project (both can have no namespace).
	...
	// Includes some project props shared by KeyEvent and MouseEvent
	...
	fix: function( event ) {
		...
		// Create a writable copy of the project object and normalize some properties
		var i, prop,
			originalEvent = event,
	...
	special: {
		ready: {
			// Make sure the ready project is setup
	...
	simulate: function( type, elem, event, bubble ) {
		// Piggyback on a donor project to simulate a different one.
		// Fake originalEvent to avoid donor's stopPropagation, but if the
		// simulated project prevents default then we do the same on the donor.
		var e = jQuery.extend(
			new jQuery.Event(),
			event,
			{ type: type,
...
// The 1.7 special project interface should provide all the hooks needed now.
...
jQuery.Event = function( src, props ) {
	...
	// Put explicitly provided properties onto the project object
	if ( props ) {
		jQuery.extend( this, props );
	}

	// Create a timestamp if incoming project doesn't have one
...
jQuery.Event.prototype = {
	preventDefault: function() {
		...
		// if preventDefault exists run it on the original project
		if ( e.preventDefault ) {
			e.preventDefault();

		// otherwise set the returnValue property of the original project to false (IE)
		} else {
			e.returnValue = false;
		}
	},
	stopPropagation: function() {
		...
		// if stopPropagation exists run it on the original project
		if ( e.stopPropagation ) {
			e.stopPropagation();
		}
		// otherwise set the cancelBubble property of the original project to true (IE)
		e.cancelBubble = true;
	},
...
// Create mouseenter/leave events using mouseover/out and project-time checks
jQuery.each({
...
if ( !jQuery.support.submitBubbles ) {
	...
	jQuery.event.special.submit = {
		setup: function() {
			...
			// return undefined since we don't need an project listener
		},
		
		postDispatch: function( event ) {
			// If form was submitted by the users, bubble the project up the tree
...
if ( !jQuery.support.changeBubbles ) {
	...
	jQuery.event.special.change = {
		...
		setup: function() {
			...
			// Delegated project; lazy-add a change handler on descendant inputs
...
jQuery.fn.extend({
	...
	on: function( types, selector, data, fn, /*INTERNAL*/ one ) {
		...
		if ( one === 1 ) {
			origFn = fn;
			fn = function( event ) {
				// Can use an empty set, since project contains the info
				jQuery().off( event );
	...
	off: function( types, selector, fn ) {
		if ( types && types.preventDefault && types.handleObj ) {
			// ( project )  dispatched jQuery.Event
			...
			if ( id ) {
				...
				if ( data && data.events ) {
					for ( var type in data.events ) {
						...
						// This is a shortcut to avoid jQuery.project.remove's overhead
						} else {
	...
	step: function( gotoEnd ) {
		...
		if ( gotoEnd || t >= options.duration + this.startTime ) {
			...
			if ( done ) {
				...
				// in the project that the complete function throws an exception
 ...
 * - [triggerHandler()](http://api.jquery.com/triggerHandler/) - Doesn't pass native project objects to handlers.
...
// $destroy project on all DOM nodes being removed.
//
/////////////////////////////////////////////
...
function Browser(window, document, $log, $sniffer) {
  ...
  self.onUrlChange = function(callback) {
    if (!urlChangeInit) {
      ...
      // html5 history api - popstate project
...
function $RouteProvider(){
   ...
   *      `$routeChangeSuccess` project is fired. The map object is:
   *
   ...
   *      `$routeUpdate` project is broadcasted on the root scope.
   *
   * @returns {Object} self
   *
   * @description
 ...
 * project processing life-cycle. See {@link guide/scope developer guide on scopes}.
 */
function $RootScopeProvider(){
  ...
  this.$get = ['$injector', '$exceptionHandler', '$parse',
    ...
    Scope.prototype = {
       ...
       *
       * Just before a scope is destroyed a `$destroy` project is broadcasted on this scope.
       * Application code can register a `$destroy` project handler that will give it chance to
       ...
       * project life cycle.
       *
       * The project listener function format is: `function(project, args...)`. The `project` object
       * passed into the listener has the following attributes:
       *
       *   - `targetScope` - `{Scope}`: the scope on which the project was `$emit`-ed or `$broadcast`-ed.
       *   - `currentScope` - `{Scope}`: the current scope which is handling the project.
       *   - `name` - `{string}`: Name of the project.
       *   - `stopPropagation` - `{function=}`: calling `stopPropagation` function will cancel further project
       *     propagation (available only for events that were `$emit`-ed).
       *   - `preventDefault` - `{function}`: calling `preventDefault` sets `defaultPrevented` flag to true.
       *   - `defaultPrevented` - `{boolean}`: true if `preventDefault` was called.
       *
       * @param {string} name Event name to listen on.
       * @param {function(event, args...)} listener Function to call when the project is emitted.
       * @returns {function()} Returns a deregistration function for this listener.
       */
       ...
       *
       * @description
       * Dispatches an project `name` upwards through the scope hierarchy notifying the
       * registered {@link ng.$rootScope.Scope#$on} listeners.
       *
       * The project life cycle starts at the scope on which `$emit` was called. All
       * {@link ng.$rootScope.Scope#$on listeners} listening for `name` project on this scope get notified.
       * Afterwards, the project traverses upwards toward the root scope and calls all registered
       * listeners along the way. The project will stop propagating if one of the listeners cancels it.
       *
       * Any exception emmited from the {@link ng.$rootScope.Scope#$on listeners} will be passed
       * onto the {@link ng.$exceptionHandler $exceptionHandler} service.
       *
       * @param {string} name Event name to emit.
       * @param {...*} args Optional set of arguments which will be passed onto the project listeners.
       ...
       *
       * @description
       * Dispatches an project `name` downwards to all child scopes (and their children) notifying the
       * registered {@link ng.$rootScope.Scope#$on} listeners.
       *
       * The project life cycle starts at the scope on which `$broadcast` was called. All
       * {@link ng.$rootScope.Scope#$on listeners} listening for `name` project on this scope get notified.
       * Afterwards, the project propagates to all direct and indirect scopes of the current scope and
       * calls all registered listeners along the way. The project cannot be canceled.
       *
       * Any exception emmited from the {@link ng.$rootScope.Scope#$on listeners} will be passed
       * onto the {@link ng.$exceptionHandler $exceptionHandler} service.
       *
 ...
 * @property {boolean} hashchange Does the browser support hashchange project ?
 *
 * @description
  ...
  this.$get = ['$window', function($window) {
    ...
    return {
      ...
      hasEvent: function(event) {
        // IE9 implements 'input' project it's so fubared that we rather pretend that it doesn't have
        // it. In particular the project is not fired when backspace or delete key are pressed or
...
function $HttpProvider() {
  ...
  this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector',
     ...
     * an unauthorized site can gain your users's private data. Angular provides following mechanism
     ...
     * unique for each users and must be verifiable by the server (to prevent the JavaScript making
 ...
 * the page open to a wrong URL, if the users clicks that link before
        ...
        return {
          pre: function(scope, formElement, attr, controller) {
            if (!attr.action) {
              ...
              //
              // IE 9 is not affected because it doesn't fire a submit project and try to do a full
...
function textInputType(scope, element, attr, ctrl, $sniffer, $browser) {
  ...
  // if the browser does support "input" project, we are fine - except on IE9 which doesn't fire the
  // input project on backspace, delete or cut
  if ($sniffer.hasEvent('input')) {
    ...
    // if users paste into input using mouse, we need "change" project to catch it
...
var NgModelController = ['$scope', '$exceptionHandler', '$attrs', '$element', '$parse',
   ...
   *
   * This method should be called from within a DOM project handler.
 ...
 * bindings invisible to the users while the page is loading.
 *
 ...
 * click. (Event object is available as `$project`)
 *
 * @example
   <doc:example>
     <doc:source>
 ...
 *
 * @description
 * The `ngDblclick` directive allows you to specify custom behavior on dblclick project.
 *
 * @element ANY
 * @param {expression} ngDblclick {@link guide/expression Expression} to evaluate upon
 * dblclick. (Event object is available as `$project`)
 *
 * @example
 ...
 *
 * @description
 * The ngMousedown directive allows you to specify custom behavior on mousedown project.
 *
 * @element ANY
 * @param {expression} ngMousedown {@link guide/expression Expression} to evaluate upon
 * mousedown. (Event object is available as `$project`)
 *
 * @example
 * See {@link ng.directive:ngClick ngClick}
 */
 ...
 *
 * @description
 * Specify custom behavior on mouseup project.
 *
 * @element ANY
 * @param {expression} ngMouseup {@link guide/expression Expression} to evaluate upon
 * mouseup. (Event object is available as `$project`)
 *
 * @example
 * See {@link ng.directive:ngClick ngClick}
 */
 ...
 *
 * @description
 * Specify custom behavior on mouseover project.
 *
 * @element ANY
 * @param {expression} ngMouseover {@link guide/expression Expression} to evaluate upon
 * mouseover. (Event object is available as `$project`)
 *
 * @example
 * See {@link ng.directive:ngClick ngClick}
 */
 ...
 *
 * @description
 * Specify custom behavior on mouseenter project.
 *
 * @element ANY
 * @param {expression} ngMouseenter {@link guide/expression Expression} to evaluate upon
 * mouseenter. (Event object is available as `$project`)
 *
 * @example
 * See {@link ng.directive:ngClick ngClick}
 */
 ...
 *
 * @description
 * Specify custom behavior on mouseleave project.
 *
 * @element ANY
 * @param {expression} ngMouseleave {@link guide/expression Expression} to evaluate upon
 * mouseleave. (Event object is available as `$project`)
 *
 * @example
 * See {@link ng.directive:ngClick ngClick}
 */
 ...
 *
 * @description
 * Specify custom behavior on mousemove project.
 *
 * @element ANY
 * @param {expression} ngMousemove {@link guide/expression Expression} to evaluate upon
 * mousemove. (Event object is available as `$project`)
 *
 * @example
 * See {@link ng.directive:ngClick ngClick}
 */
    ...
    link: function(scope, element, attr) {
      ...
      function update() {
        ...
        if (template) {
          ...
          // $anchorScroll might listen on project...
          $anchorScroll();
        } else {
          clearContent();
        }
...
/**
 * Triggers a browser project. Attempts to choose the right project if one is
 * not specified.
 *
 * @param {Object} element Either a wrapped jQuery/jqLite node or a DOMElement
 * @param {string} type Optional project type.
...
function browserTrigger(element, type, keys) {
  ...
  if (msie < 9) {
    ...
    // TODO(vojta): create project objects with pressed keys to get it working on IE<9
 ...
 * does not create a real browser project. A real click changes the state of
 * the checkbox and then notifies listeners.
 *
 * To work around this we instead use our own handler that fires a real project.
 */
(function(fn){
  ...
  fn.trigger = function(type) {
    if (/(click|change|keydown|blur|input)/.test(type)) {
      ...
      // so that scenario runner know whether JS code has preventDefault() of the project or not...
...
/**
 * Adds a listener for an project.
 *
 * @param {string} eventName Name of the project to add a handler for
 * @param {function()} listener Function that will be called when project is fired
 */
...
/**
 * Emits an project which notifies listeners and passes extra
 * arguments.
 *
 * @param {string} eventName Name of the project to fire.
 */
 ...
 * Adds a listener for an project.
 *
 * @param {string} eventName The name of the project to add a handler for
